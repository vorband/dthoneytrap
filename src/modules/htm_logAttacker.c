...
int logattacker(Attack *a) {
	...
	if (fprintf(f, "[%s:%lu GMT] %s:%d -> %s:%d (%u bytes)\n",
		tstr, tv.tv_usec, shost, a->a_conn.r_port, dhost, a->a_conn.l_port, a->a_conn.payload.size) < 0) {
...
printf("--> plugin_process_confopts(): %s\n", node->keyword);
